...
> 切り替える経路（`TWIN_DATASET=the_touch`）も用意してあるが、**その画像は各自が
> ローカルで用意し、リポジトリには一切含めない**（報告するのは精度などの集計値のみ）。
> `data/` と `*.pt` 等は `.gitignore` 済み。
...
### 実写双子で A/B 学習セットを作る（任意・ローカル限定）

実在双子（例: ザ・たっち）の A/B データを手元で作る経路。**画像は非公開・リポに含めない**。
収集 → 両顔抽出 → 手動ラベル → 分割の 4 段で `data/the_touch/{train,val}/{A,B}` を作る。

```bash
.venv/bin/python scripts/fetch_the_touch.py            # 収集（ddgs 画像検索→顔フィルタ→出典CSV）
.venv/bin/python scripts/extract_faces.py              # コンビ写真から両顔を 224x224 整列クロップ
.venv/bin/python scripts/label_faces.py                # GUI で A=たくや/B=かずや/skip を手動ラベル
.venv/bin/python scripts/build_ab_split.py --clean     # data/the_touch/{train,val} を生成
```

詳細・データセット切替（`TWIN_DATASET`）は [`TECHNICALJ.md`](./TECHNICALJ.md) を参照。
...
> **データセット切替**: `TWIN_DATASET=the_touch` を前置すると、学習・評価・分類・
> リアルタイムの参照データと重みが実写セット（`data/the_touch` / `results/cnn_thetouch.pt`）に
> 一括で切り替わる（既定は合成 `synthetic`）。例:
...
#   --weights PATH で cnn の重みを差し替え（既定 cnn.pt を上書きせず別重みで評価）
...
画面取得は `--capture` で選ぶ: **X11=mss / Wayland(GNOME 等)=portal**（`auto` が自動判別）。
**既定は dry-run**（クリックせずログのみ）で安全。
...
- `--capture auto|mss|portal`: 取得/操作方式（既定 auto。Wayland は portal、X11 は mss）
- `--region X Y W H`: キャプチャ領域（既定はキャプチャ対象＝モニタ全体）
- `--a-xy` / `--b-xy`: A/B 判定時のクリック先（既定は領域の左1/3・右2/3）
- `--mode explain` は `--refs-dir`（`A/` `B/` を含む）必須

> Wayland では mss が全黒・pynput クリックが届かないため、`portal` 方式
> （xdg-desktop-portal ScreenCast 取得 + RemoteDesktop 注入）を使う。実行中に GNOME の
> 「画面共有＋操作」許可ダイアログが出るので承認すること。

---

## 8. 合成双子デモを一括起動（tmux）

GUI（合成 val 顔 + A/B ボタン）と自動プレイ（CNN 40枚 → VLM few-shot 6枚）を tmux で一括起動/停止。

```bash
./start_all.sh        # GUI + (必要なら)VLMサーバ + 自動プレイ CNN(40)->VLM(6)
tmux attach -t twin-demo   # ライブ確認（Ctrl-b 0/1/2 で切替, Ctrl-b d でデタッチ）
./stop_all.sh         # 一括停止（既存サーバは据置。止めるなら ./stop_all.sh --server）
```

- GUI は CNN/VLM のスコアを分離表示し、判定中のモード（CNN/VLM）を大きく示す。
- Wayland では実行中に「画面共有＋操作」許可が CNN/VLM で各1回出る。承認後はマウスに触れない。
...
| realtime のキャプチャが真っ黒 | Wayland。`--capture portal`（auto なら自動）で取得し、共有ダイアログを承認 |
| realtime で実クリックしない | 既定 dry-run のため `--no-dry-run`。Wayland は portal 注入、X11 は pynput |
