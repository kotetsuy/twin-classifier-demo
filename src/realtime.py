...
X11 前提（DISPLAY 必要）。pynput でマウス制御、なければ自動的に dry-run。
...
class Clicker:
    """pynput があれば実クリック、なければ（または dry_run なら）ログのみ。"""
    ...
    def __init__(self, dry_run: bool):
        ...
        self.dry_run = self.controller is None
...
def run(args) -> None:
    ...
    clicker = Clicker(args.dry_run)

    with mss.MSS() as sct:
        if region is None:  # 既定はプライマリモニタ全体
            m = sct.monitors[1]
        ...
        print(f"region={region}  A->click{a_xy}  B->click{b_xy}  "
              f"mode={args.mode}  {'DRY-RUN' if clicker.dry_run else 'LIVE-CLICK'}")

        frame_no = 0
        try:
            while args.max_frames == 0 or frame_no < args.max_frames:
                frame_no += 1
                shot = np.asarray(sct.grab(bbox))[:, :, :3]  # BGRA->BGR
                t = time.time()
